/* initialise variable */
var ws,
  syms = document.getElementById("selectSyms"),
...
function setSyms(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    syms.innerHTML +=
      '<input type="checkbox" name="sym" value="' +
      data[i] +
      '">' +
      data[i] +
      "</input>";
...
function setQuotes(data) {
  quotes.innerHTML = generateTableHTML(data);
}
function setTrades(data) {
  trades.innerHTML = generateTableHTML(data);
}
...
function generateTableHTML(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    for (var x in data[0]) {
      ...
      var cellData;
      if ("time" === x) cellData = data[i][x].substring(2, 10);
      else if ("number" == typeof data[i][x]) cellData = data[i][x].toFixed(2);
